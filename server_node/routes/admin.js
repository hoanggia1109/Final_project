...
// Multer upload cho ảnh sản phẩm/biến thể
...
router.put("/bienthe/:id", auth, upload.array("images", 10), async (req, res) => {

  try {

    const { mausac, sl_tonkho, gia, kichthuoc, chatlieu, deletedImages } = req.body;

    // Cập nhật thông tin biến thể
    const updateData = {};
    if (mausac !== undefined) updateData.mausac = mausac;
    if (sl_tonkho !== undefined) updateData.sl_tonkho = sl_tonkho;
    if (gia !== undefined) updateData.gia = gia;
    if (kichthuoc !== undefined) updateData.kichthuoc = kichthuoc;
    if (chatlieu !== undefined) updateData.chatlieu = chatlieu;

    await SanPhamBienTheModel.update(updateData, { where: { id: req.params.id } });

    // Xóa ảnh cũ nếu có danh sách ảnh cần xóa
    if (deletedImages) {
      let imageIdsToDelete = [];
      try {
        imageIdsToDelete = typeof deletedImages === 'string' 
    ...
    if (req.files && req.files.length > 0) {
      const newImages = req.files.map((file) => ({
        id: uuidv4(),
        bienthe_id: req.params.id,
        url: `/uploads/sanpham/${file.filename}`,
      }));

      await ImageModel.bulkCreate(newImages);
      console.log(`Added ${newImages.length} new images for variant ${req.params.id}`);
    }

    // Lấy lại biến thể với ảnh để trả về
    const updatedBienthe = await SanPhamBienTheModel.findByPk(req.params.id, {
      include: [
        {
          model: ImageModel,
          as: "images",
...
router.get("/donhang/:id", auth, isAdmin, async (req, res) => {
  ...
  try {
    ...
    const { DiaChiModel, SanPhamBienTheModel, SanPhamModel } = require("../database");

    console.log(`[GET /api/admin/donhang/:id] Fetching order ${req.params.id}`);
    ...
    if (!dh) {
      console.log(`[GET /api/admin/donhang/:id] Order ${req.params.id} not found`);
      return res.status(404).json({ message: "Không tìm thấy đơn hàng" });
    }

    // Log để debug
    console.log(`[GET /api/admin/donhang/:id] Order found: ${dh.code}`);
    console.log(`[GET /api/admin/donhang/:id] User ID: ${dh.user_id}`);
    console.log(`[GET /api/admin/donhang/:id] User in instance:`, dh.user ? 'YES' : 'NO');
    
    // Nếu user không được include, query riêng
    let userData = null;
    if (!dh.user && dh.user_id) {
      console.log(`[GET /api/admin/donhang/:id] User not included, fetching separately...`);
      const user = await UserModel.findByPk(dh.user_id, {
        attributes: ["id", "email", "ho_ten", "sdt"]
      });
      if (user) {
        userData = user.get({ plain: true });
        console.log(`[GET /api/admin/donhang/:id] User fetched separately:`, userData.email);
      }
    } else if (dh.user) {
      userData = dh.user.get ? dh.user.get({ plain: true }) : dh.user;
      console.log(`[GET /api/admin/donhang/:id] User from include:`, userData.email);
    }

    // Convert to plain object để đảm bảo Sequelize serialize đúng
    const orderData = dh.get({ plain: true });
    
    // Đảm bảo user data được thêm vào
    if (userData) {
      orderData.user = userData;
    }
    
    console.log(`[GET /api/admin/donhang/:id] Final order data has user:`, !!orderData.user);
 ...
 * - include_orders (optional): true để bao gồm danh sách đơn hàng
