...
export default function TonKhoPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const url = `http://localhost:5001/api/tonkho?page=${currentPage}&limit=${itemsPerPage}`;
  ...
  const loadThongKe = async () => {
    try {
      const token = localStorage.getItem('token');
      const res = await fetch('http://localhost:5001/api/tonkho/thongke/summary', {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) {
  ...
  const handleNhapKho = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch('http://localhost:5001/api/tonkho/nhap', {
        method: 'POST',
        headers: {
  ...
  const handleXuatKho = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch('http://localhost:5001/api/tonkho/xuat', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
