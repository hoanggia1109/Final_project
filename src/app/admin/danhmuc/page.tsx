...
export default function AdminDanhMucPage() {
  ...
  useEffect(() => {
    fetch('http://localhost:5001/api/danhmuc?admin=true')
      .then(res => res.json())
      .then(data => {
        setList(data);
        setLoading(false);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`http://localhost:5001/api/danhmuc/${id}`, { method: 'DELETE' });
      if (res.ok) {
  ...
  return (
    <div>
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          <h2 className="fw-bold mb-1" style={{ color: '#2C3E50' }}>Quản lý Danh mục</h2>
          <p className="text-muted mb-0">Danh sách tất cả các danh mục sản phẩm</p>
        </div>
        <button
          className="btn btn-warning text-white d-flex align-items-center gap-2 shadow-sm"
          onClick={() => router.push('/admin/danhmuc/create')}
        >
          <PlusCircle size={18} />
            ...
            <table className="table table-hover mb-0">
              <thead style={{ backgroundColor: '#F8F9FA' }}>
                <tr>
                  ...
                  <th className="px-4 py-3 text-center" style={{ width: '120px' }}>Mã</th>
                  ...
                  filtered.map((item, index) => (
                    <tr key={item.id}>
                      ...
                      <td className="px-4 py-3">
                        <span className="fw-semibold" style={{ color: '#2C3E50' }}>{item.tendm}</span>
                      </td>
                      ...
                      <td className="px-4 py-3 text-center">
                        <div className="d-flex justify-content-center gap-2">
                          <button
                            onClick={() => router.push(`/admin/danhmuc/edit/${item.id}`)}
