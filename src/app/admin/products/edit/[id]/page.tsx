...
export default function EditProductPage() {
  ...
  useEffect(() => {
    ...
    if (!productId) return;
    fetch(`http://localhost:5001/api/sanpham/${productId}`)
      .then(res => res.json())
      .then((data) => {
        setForm({
          code: data.code,
          ...
          thumbnail: data.thumbnail ? { url: `http://localhost:5001${data.thumbnail}` } : null,
        });
        ...
        const variants = data.bienthe.map((bt: any) => ({
          ...
          images: bt.images.map((img: any) => ({ url: `http://localhost:5001${img.url}` })),
        }));
        setBienthe(variants);
      });
  }, [productId]);
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:5001/api/sanpham/${productId}`, {
        method: 'PUT',
        body: formData,
      });
